...
NODE_TYPES: list[NodeTypeInfo] = [
    ...
    NodeTypeInfo(
        ...
        icon="arrow-down-up",
        category="rag",
        description="Rerank retrieved documents by relevance to the query; keep the best top_n.",
        config_schema=RerankNodeConfig.model_json_schema(),
        inputs=[
            PortDecl(name="query", type="text", label="Query"),
            PortDecl(name="documents", type="documents", label="Documents"),
        ],
        outputs=[PortDecl(name="documents", type="documents", label="Documents")],
        default_config={"top_n": 4},
        ui={
            "resource": FieldUI(
                widget="resource",
                label="Reranker",
                resource_kind="model_provider",
                help="Model provider whose base_url serves a /rerank endpoint (Cohere/Jina/TEI).",
            ),
            "model": FieldUI(widget="text", label="Model", placeholder="e.g. rerank-english-v3.0"),
            "top_n": FieldUI(widget="number", label="Top N"),
            "min_score": FieldUI(
                widget="number",
                label="Min Score",
                help="Rerank score threshold — documents scoring below it are dropped.",
                advanced=True,
            ),
        },
    ),
