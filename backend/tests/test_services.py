...
@pytest.fixture
async def sqlite_stack(sqlite_settings):
...
# ------------------------------------------------------------------ secrets (§10.3)
async def test_credentials_encrypted_and_write_only(sqlite_stack):
    ...
    assert row.value_encrypted and "sk-secret" not in row.value_encrypted
    ...
    assert provider.has_var("region") and not provider.has_var("missing")


async def test_env_promotion(sqlite_stack, monkeypatch):
    settings, sessions = sqlite_stack
...
# ------------------------------------------------------------------ push (§7.9)
def test_ssrf_validation(sqlite_settings):
...
async def test_push_delivery_with_token_and_retry(sqlite_stack):
    settings, sessions = sqlite_stack
    ...
    assert payload["id"] == "pt1" and payload["status"]["state"] == "completed"
    await client.aclose()


async def test_push_blocked_for_private_url_by_default(sqlite_stack):
    settings, sessions = sqlite_stack
    store = DbPushConfigStore(sessions, settings)
...
# ------------------------------------------------------------------ files (§9.6)
async def test_files_roundtrip(sqlite_stack):
    ...
    assert info["size"] == 11 and "token=" in info["url"]
